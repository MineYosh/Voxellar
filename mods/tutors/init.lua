...
TTL_MSG = 13       -- how long messages stay up
TTL_NEXT = 0.1       -- how long in between messages
...
local function hud_display(player, data, item)
	...
	if data.complete[item] then
		description = "Nice! You have completed the goal:\n\"" .. goal.text .. "\"!\n\n"
...
local function hud_update(player, data)
	...
	if data.hud_open then
		...
		data.ttl = TTL_NEXT
		minetest.after(0.5, hud_update, player, data)
		return
	end
	...
	for k, _ in pairs(data.complete) do
		...
		data.complete[k] = nil
		minetest.after(0.20, hud_update, player, data)
		return
	end
	...
	for k, _ in pairs(data.new) do
		...
		hud_display(player, data, k)
		data.new[k] = nil
		minetest.after(0.2, hud_update, player, data)
		return
	end
		...
		for k, _ in pairs(data.active) do
			if data.last_active == k then
				...
				if k then
					...
					data.last_active = k
					hud_display(player, data, k)
					minetest.after(0.1, hud_update, player, data)
					return
				end
